...
std::ofstream avgThptFileVsPer;
std::ofstream avgThptFileVsBw;
...
TraceNextRx (std::string &next_rx_seq_file_name)
{
  ...
  Config::ConnectWithoutContext ("/NodeList/11/$ns3::TcpL4Protocol/SocketList/1/RxBuffer/NextRxSequence", MakeCallback (&NextRxTracer));
}

int main (int argc, char *argv[])
{
  
  std::string transport_prot = "TcpPetra";
  realtimeThroughputFile.open ("data/realtimeThroughput"+transport_prot);
  avgThptFileVsPer.open ("data/avgThroughputVsPer"+transport_prot,std::ios_base::app);
  avgThptFileVsBw.open ("data/avgThroughputVsBw"+transport_prot,std::ios_base::app);
  ...
  // Select TCP variant
  if (transport_prot.compare ("ns3::TcpWestwoodPlus") == 0)// 0 means strings are equal
    { 
    ...
    else if(transport_prot.compare ("ns3::TcpPetra") == 0) 
    {
      // TcpWestwoodPlus is not an actual TypeId name; we need TcpWestwood here
      Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpWestwood::GetTypeId ()));
      // the default protocol type in ns3::TcpWestwood is WESTWOOD
      Config::SetDefault ("ns3::TcpWestwood::ProtocolType", EnumValue (TcpWestwood::PETRA));
    }
  else
    {
      TypeId tcpTid;
  ...
  for (uint32_t i = 0; i < num_flows; i++)
    {
      ...
      NS_LOG_INFO("local link "<<interfaces.GetAddress (0)<<" "<<interfaces.GetAddress (1));
      ...
      NS_LOG_INFO("unrelink link "<<interfaces.GetAddress (0)<<" "<<interfaces.GetAddress (1));
  ...
  if (tracing)
    {
      ...
      Simulator::Schedule (Seconds (0.00001), &TraceCwnd, prefix_file_name + "-10cwnd.data");
  ...
  avgThptFileVsPer << error_p <<"\t" <<averageThroughput << std::endl;
  avgThptFileVsBw<<bandwidth<<"\t"<<averageThroughput<<std::endl;
  return 0;
}
